...
export default {
    ...
    filters: {
        ...
        guestType: function (role) {
            if (role === "guestwithoutinvite") return "Without Invite";
            else return "With Invite";
        },
    ...
    created: function () { },
    mounted: function () {
        this.applianceId = this.$store.state.chosenAppliance.applData.applianceId;
        this.setTableMsg();
        this.renderApplData();
    ...
    methods: {
        ...
        setTableMsg: function () {
            this.tableMsg = "Loading guest user list";
        },
        ...
        getUserAccessList: function () {
            if (this.$userRoleAccess(this.applianceId, 'permitApprovedGuestList')) {
                this.inlineAppLoader = true;
                this.userSearch = "";
                let callBackFunc = this.renderUserAccessList.bind(this);
        ...
        renderUserAccessList: function () {
            ...
            let userList = this.$store.state.userAccessList.userAccessListGuest.userInformation;
            let userPermissionInfo = this.$store.state.userAccessList.userAccessListGuest.userPermissionInfo;
            if (!userList.length) {
                this.tableMsg = "";
            }

            for (let i = 0; i < userList.length; i++) {
                let permissionInfo = _.find(userPermissionInfo, {
            ...
            console.log(userList, "updated values with permissionInfo");
            ...
            for (let i = 0; i < userList.length; i++) {
                this.userItems.push({
                    ...
                    expand: "",
                    name:
                        userList[i].userData.firstName +
                        " " +
                        userList[i].userData.lastName,
                    ...
                    guestType: userList[i].permissionInfo.accessTypeName
                });
            }
            this.listLength = this.userItems.length;
            this.$root.$emit("guest_list_length", this.listLength);
        },
        //-------------------------------------
